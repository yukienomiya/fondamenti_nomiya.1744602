...
def es22(filePng, fileJson):
  ...
  for row in range(len(img)):
    ...
    for col in range(len(img[0])):
      ...
      for comp in pixel:
        comp = str(comp)
        addedZeros = '0' * (3 - len(comp))
        pixelStr += addedZeros + comp
      occurrencesDict[pixelStr] = occurrencesDict.get(pixelStr, 0) + 1
      matrixRow.append(pixelStr)
    m.append(matrixRow)

  fJSON = open(fileJson, 'w')
  json.dump(m, fJSON)
